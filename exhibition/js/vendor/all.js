...
(function ($) {
		...
		this.each(function() {
			...
			*/
			...
			var Modern = {
				...
				preload: function(n){
					// Preload all of the img but never show them, just use their completion so we know that they are done
			...
			var Old = {
				setup: function(){
					...
					if($.BrowserTests.msie && !config.overrideMSIEStop){
						// Stop IE from continually trying to preload img that we already removed
					...
					if($.Slides.length > 0){
						...
						for(var j in $.Slides) {
							...
							// Account for slides without img
				...
				preload: function(){
					// Intervals to tell if an img have loaded
					var t = setInterval(function() {
						$('.mc-loader').children('img').each(function(i){
							// Check if image is loaded
							var $img = $(this);
							
							// Loop through not-loaded img
						...
						if( $('.not-loaded').length == 0){
							// Remove our loader element because all of our img are now loaded
							$('.mc-loader').remove();
							
							// Clear interval when all img are loaded
							clearInterval(t);
						}
					}, 1000);
				},
	...
	$.fn.maximage.defaults = {
		...
		overrideMSIEStop: false, // This gives the option to not 'stop' load for MSIE (stops coded background img from loading so we can preload)...
